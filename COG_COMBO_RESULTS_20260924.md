...
## 第三轮 screen3（n=8）要点
- O+B 堆叠（每 2 步跳 1 步，其余真实步中每 3 步里 2 步复用全部块）的质量主要由起始步 w 决定：
  - w=10：2.07×，PSNR 29.1（在扰动尺度内）；加 GI 为 2.20×/28.5。
  - w=8：2.37×，PSNR 26.1；加 GI 为 2.46×/25.9。
  - w=6：2.80×，PSNR 22.4；加 GI 为 2.92×/22.5。
- 同速减步基线：steps9 为 3.35×/12.9，steps8 为 3.76×/12.9，ΔCLIP 分别为 −2.6 与 −4.1，运动比约 0.5。组合在同速下 PSNR 高 7–13 dB，而且不掉运动。
- 只用块复用（S3_Lall_k6_w6）为 2.53×/22.6，不如 O+B 的 S3_O6_B3（2.80×/22.4）。只复用中间块（Lmid）反而更差（1.94×/22.5），说明首尾块并不是瓶颈。
- 目前 Pareto 前沿大约为：2.1×@29、2.4×@26、2.9×@23、3.3×@20。
