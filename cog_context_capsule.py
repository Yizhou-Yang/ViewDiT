...
OUT=Path('/root/viewdit/results/video/cog_context_capsule')
...
def packed(t,dtype=torch.float32):
    ...
    array=t.view(torch.int16).numpy() if dtype==torch.bfloat16 else t.numpy()
    return zlib.compress(array.tobytes(order='C'),level=9)
    ...
    for clip in report['protocol']['clips']:
        ...
        for amplitude in [.4,1.]:
            ...
            for size in [(4,7),(8,14),(16,28),(32,56)]:
                ...
                correction=F.interpolate(coeff.float(),size=(128,224),mode='bilinear',align_corners=False).permute(1,0,2,3).unsqueeze(0)
