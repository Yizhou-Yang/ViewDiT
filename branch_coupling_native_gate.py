...
def main():
    ...
    report = {'complete': False, 'gpu': torch.cuda.get_device_name(), 'torch': torch.__version__, 'script_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest(), 'protocol': {'h':480,'w':720,'frames':17,'steps':50,'strength':.75,'cfg':6.,'seeds':list(range(20261001,20261003)),'prompts':{c:CASES[c] for c in ['bear','bus']},'method':'untrained V2V with DDIM eta0; no prefix pins','limits':'two development clips; native spatial resolution but short17 frames; no semantic success metric; border proxy not mask; assignment is standard baseline'}, 'clips': {}, 'sampling': []}
    def save():
    ...
    for clip in ['bear', 'bus']:
        x = load_source(args.data, clip, 480, 720)
    ...
    all_latents = {}
    for clip, prompts in [(c, CASES[c]) for c in ['bear', 'bus']]:
        ...
        for branch, prompt in enumerate(prompts):
            ...
            for seed in report['protocol']['seeds']:
                ...
                result = pipe(latents=initial, prompt_embeds=embeds[prompt].cuda(), negative_prompt_embeds=embeds[''].cuda(), height=480, width=720, num_inference_steps=50, strength=.75, guidance_scale=6., eta=0., generator=gen, output_type='latent').frames
