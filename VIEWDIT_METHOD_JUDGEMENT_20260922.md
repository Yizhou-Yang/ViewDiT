...
## 撤回说明（2026-09-22 后续审查）

下文作为历史错误判定保留，不再作为有效研究结论。用户明确允许训练，免训练不是硬性约束。

1. 单个bear、strength=0.6、整体亮度/chroma不能证伪冻结模型语义编辑能力。SEMANTIC_V3_STATUS.md的高强度补测已经记录strength=0.85下可辨认白熊，但背景姿态漂移；其末尾随后作出的普遍否定同样不成立。
2. protect=none脚本在去噪结束后仍执行ze[:,:,:start_latent]=z[:,:,:start_latent]，不是完全无保护的端到端协议；它也没有实现被声称证伪的attention注入方法。
3. capsule的edited_max=0由保留raw后缀并像素拼接构造得到，不能用于推断decoder无时域耦合。edit_amp接近1只说明幅度接近，不能说明编辑内容不变。
4. 下表的conflict_scan属于Cog实验，被错误归给Wan；GN统计capsule与Latte的SVD前缀差值缓存是不同机制，不能直接合并平凡性结论。
5. SIGE/Tiled VAE等先例使单纯统计重放的novelty不足，但不存在由一个失败样例推出整个编辑研究方向无法成为顶会成果的证据。LoRA权重更新也不等价于latent低秩投影。
6. 当前有效结论：限定配置的参考恢复有实验支持，语义质量收益、跨架构普适性和端到端significance尚未建立；后续允许训练并重新审查方法与强基线。
